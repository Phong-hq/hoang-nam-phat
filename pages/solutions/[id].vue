      ...
      <div class="bg-white border-b border-gray-200 py-8">
        <div class="container mx-auto px-4">
          <nav class="text-sm text-gray-400 mb-3 flex items-center gap-1.5">
            ...
            <span>/</span>
            <NuxtLink to="/solutions" class="hover:text-primary transition-colors">Giải pháp</NuxtLink>
            <span>/</span>
...
const { data: item, pending, error } = await useAsyncData(
  `solutions-${id.value}`,
  async () => {
    await solutionsPageStore.fetchSolutionsList()
    return solutionsPageStore.getById(id.value) ?? null
  },
  { watch: [id] },
)

if (error.value || !item.value) {
  throw createError({ statusCode: 404, statusMessage: 'Không tìm thấy bài viết giải pháp' })
}
